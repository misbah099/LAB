...
select last_name, job_id, employee_id from EMPLOYEE where EMPLOYEE_ID =198;
...
#6 6. Display unique job_id from EMPLOYEE table. Give alias name to the column as JOB_TITLE.
...
select LAST_NAME,SALARY,salary+100 as 'Annual comp' from Employee;
...
select Employee_id, Department_id,Salary from EMPLOYEE where salary >=4000 AND salary <=7000 ;
#13 .Display the details of all employees whose salary is either 6000 or 6500 or 7000.
select Employee_id, Department_id,Salary from EMPLOYEE where salary=6000 OR salary=6500 OR salary=7000 ;
...
update EMPLOYEE SET JOB_TITLE='GRADE A' where salary>=5000;
